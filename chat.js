...
const gameState = {
    ...
    lastFedCheck: 0,
    fedRaidTimer: null,
    fedAlertActive: false
};

// Enhanced Game Items with new items
const items = {
    drugs: {
    ...
    jobs: [
        { name: 'Fast Food Worker', pay: 50, risk: 0 },
        { name: 'Construction Worker', pay: 100, risk: 10 },
        { name: 'Security Guard', pay: 200, risk: 30 },
        { name: 'Bodyguard', pay: 500, risk: 50 }
    ],
    businesses: [
        { name: 'Car Wash', cost: 10000, launderCapacity: 1000 },
        { name: 'Restaurant', cost: 50000, launderCapacity: 5000 },
        { name: 'Nightclub', cost: 200000, launderCapacity: 20000 }
    ],
    slotSymbols: ['🍒', '🍋', '🍊', '🍇', '🍉', '7️⃣', '💎'],
    rouletteNumbers: [
...
const elements = {
    ...
    fedAlert: document.getElementById('fed-alert'),
    fedTimer: document.getElementById('fed-timer')
};
...
function updateUI() {
    ...
    const displayHour = gameState.time > 12 ? gameState.time - 12 : gameState.time === 0 ? 12 : gameState.time;
    ...
    // Check for fed attention if not in jail and not already being raided
    if (!gameState.inJail && !gameState.fedAlertActive && gameState.day !== gameState.lastFedCheck) {
        checkFedAttention();
    }
}
...
function openLocation(location) {
    ...
    switch(location) {
        ...
        case 'jobs':
            elements.locationContent.innerHTML = `
                <h2>Jobs</h2>
                <p>Earn legitimate income to avoid suspicion.</p>
                <button id="open-jobs">View Jobs</button>
            `;
        ...
        case 'businesses':
            elements.locationContent.innerHTML = `
                <h2>Businesses</h2>
                <p>Launder money through legitimate businesses.</p>
                <button id="open-businesses">View Businesses</button>
            `;
            
            document.getElementById('open-businesses').addEventListener('click', () => {
...
// Fed Attention System
function checkFedAttention() {
    gameState.lastFedCheck = gameState.day;
    
    // Check if player has too much cash without laundering
    if (gameState.cash > 50000 && gameState.businesses.length === 0) {
        gameState.fedAttention += Math.floor(gameState.cash / 10000);
        addNotification("The feds are noticing your large cash holdings...");
    }
    
    // Check for illegal activities that increase fed attention
    if (gameState.drugInventory.length > 10 || gameState.guns.length > 2) {
        gameState.fedAttention += 5;
    }
    
    // Random event based on fed attention
    if (gameState.fedAttention > 0 && Math.random() * 100 < gameState.fedAttention) {
        startFedAlert();
    }
}

function startFedAlert() {
    if (gameState.fedAlertActive || gameState.inJail) return;
    
    gameState.fedAlertActive = true;
    elements.fedAlert.classList.remove('hidden');
    
    let timeLeft = 30;
    elements.fedTimer.textContent = `Time until raid: ${timeLeft}s`;
    ...
    gameState.fedRaidTimer = setTimeout(() => {
        clearInterval(countdown);
        startFedRaid();
    }, 30000);
}

function cancelFedAlert() {
    if (!gameState.fedAlertActive) return;
    
    clearTimeout(gameState.fedRaidTimer);
    gameState.fedAlertActive = false;
    elements.fedAlert.classList.add('hidden');
    gameState.fedAttention = Math.max(0, gameState.fedAttention - 10);
    addNotification("You avoided the fed raid!");
}

// Fed Raid Mini-Game
function setupFedRaid() {
    const hidingSpots = document.querySelectorAll('.hiding-spot');
    hidingSpots.forEach(spot => {
        spot.addEventListener('click', () => {
            const drug = document.querySelector('.drug-to-hide');
            if (drug) {
                spot.appendChild(drug);
                checkFedRaidCompletion();
            }
        });
    });
}

function startFedRaid() {
    gameState.fedAlertActive = false;
    elements.fedAlert.classList.add('hidden');
    
    if (gameState.drugInventory.length === 0 && gameState.craftedStrains.length === 0) {
        addNotification("Feds raided but found nothing! Suspicion decreased.");
        gameState.fedAttention = 0;
        return;
    }
    
    document.getElementById('fed-raid-modal').style.display = 'block';
    const drugsContainer = document.getElementById('drugs-to-hide');
    drugsContainer.innerHTML = '';
    
    // Create drug elements to hide
    gameState.drugInventory.forEach(drug => {
        const drugElement = document.createElement('div');
        drugElement.className = 'drug-to-hide';
        drugElement.textContent = drug.name;
        drugElement.setAttribute('data-drug', drug.name);
    ...
    gameState.craftedStrains.forEach(strain => {
        const drugElement = document.createElement('div');
        drugElement.className = 'drug-to-hide';
        drugElement.textContent = strain.name;
        drugElement.setAttribute('data-drug', strain.name);
        drugsContainer.appendChild(drugElement);
    });
    
    document.getElementById('fed-raid-status').textContent = `Hide all ${drugsContainer.children.length} drugs!`;
}

function checkFedRaidCompletion() {
    const remainingDrugs = document.querySelectorAll('.drug-to-hide').length;
    document.getElementById('fed-raid-status').textContent = `Drugs left to hide: ${remainingDrugs}`;
    
    if (remainingDrugs === 0) {
        // Success - some drugs may still be found
        const foundChance = 30; // 30% chance per drug to be found
        let foundCount = 0;
        
        gameState.drugInventory.forEach(drug => {
            if (Math.random() * 100 < foundChance) {
                foundCount += drug.amount;
                gameState.drugInventory = gameState.drugInventory.filter(d => d.name !== drug.name);
            }
        });
        ...
        gameState.craftedStrains.forEach(strain => {
            if (Math.random() * 100 < foundChance) {
                foundCount++;
                gameState.craftedStrains = gameState.craftedStrains.filter(s => s.name !== strain.name);
            }
        });
        
        if (foundCount > 0) {
            addNotification(`Feds found ${foundCount} drugs! The rest were safely hidden.`);
            getArrested("Drug possession");
        } else {
            addNotification("You successfully hid all your drugs from the feds!");
        }
        
        document.getElementById('fed-raid-modal').style.display = 'none';
        gameState.fedAttention = 0;
    }
}
    ...
    updateJobs = function() {
        ...
        if (gameState.currentJob) {
            currentJobDiv.innerHTML = `
                <h3>Current Job: ${gameState.currentJob.name}</h3>
                <p>Pay: $${gameState.currentJob.pay}/day</p>
                <p>Risk: ${gameState.currentJob.risk}%</p>
                <button id="quit-job">Quit Job</button>
            `;
            
            document.getElementById('quit-job').addEventListener('click', () => {
                addNotification(`Quit your job as ${gameState.currentJob.name}`);
                gameState.currentJob = null;
                updateJobs();
            });
        } else {
            currentJobDiv.innerHTML = '<h3>Current Job: None</h3>';
        }
    };
    
    document.querySelectorAll('.take-job').forEach(btn => {
        btn.addEventListener('click', function() {
            const jobDiv = this.parentElement;
            const jobName = jobDiv.querySelector('h3').textContent;
            const pay = parseInt(jobDiv.getAttribute('data-pay'));
            const risk = parseInt(jobDiv.getAttribute('data-risk'));
            
            if (gameState.currentJob) {
                addNotification("You already have a job! Quit your current job first.");
                return;
            }
            
            gameState.currentJob = { name: jobName, pay: pay, risk: risk };
            addNotification(`Started working as ${jobName}. You'll earn $${pay} each day.`);
            updateJobs();
        });
    });
}

// Business and Money Laundering System
function setupBusinesses() {
    updateBusinesses = function() {
        const businessList = document.getElementById('business-list');
        businessList.innerHTML = '';
        
        if (gameState.businesses.length === 0) {
            businessList.innerHTML = '<p>No businesses owned</p>';
            document.getElementById('launder-money').disabled = true;
            return;
        }
        
        gameState.businesses.forEach((business, index) => {
            const bizElement = document.createElement('div');
            bizElement.className = 'business-owned';
            bizElement.innerHTML = `
                <h4>${business.name}</h4>
                <p>Launder Capacity: $${business.launderCapacity}/day</p>
                <button class="sell-business" data-index="${index}">Sell (${Math.floor(business.cost * 0.7)})</button>
            `;
            businessList.appendChild(bizElement);
        });
        
        document.getElementById('launder-money').disabled = false;
        
        document.querySelectorAll('.sell-business').forEach(btn => {
            btn.addEventListener('click', function() {
                const index = parseInt(this.getAttribute('data-index'));
                const business = gameState.businesses[index];
                const sellPrice = Math.floor(business.cost * 0.7);
                
                gameState.cash += sellPrice;
                gameState.businesses.splice(index, 1);
                addNotification(`Sold ${business.name} for $${sellPrice}`);
                updateBusinesses();
                updateUI();
            });
        });
    ...
    document.querySelectorAll('.buy-business').forEach(btn => {
        btn.addEventListener('click', function() {
            const businessDiv = this.parentElement;
            const businessName = businessDiv.querySelector('h3').textContent;
            const cost = parseInt(businessDiv.getAttribute('data-cost'));
            const launderCapacity = parseInt(businessDiv.getAttribute('data-launder'));
            
            if (gameState.cash < cost) {
                addNotification("You can't afford this business!");
                return;
            }
            
            if (gameState.businesses.some(b => b.name === businessName)) {
                addNotification("You already own this type of business!");
                return;
            }
            
            gameState.cash -= cost;
            gameState.businesses.push({
                name: businessName,
                cost: cost,
                launderCapacity: launderCapacity
            });
            
            addNotification(`Purchased ${businessName} for $${cost}!`);
            updateBusinesses();
            updateUI();
        });
    });
    
    document.getElementById('launder-money').addEventListener('click', function() {
        if (gameState.businesses.length === 0) return;
        
        let totalCapacity = gameState.businesses.reduce((sum, biz) => sum + biz.launderCapacity, 0);
        const amountToLaunder = Math.min(gameState.cash, totalCapacity);
        
        if (amountToLaunder <= 0) {
            document.getElementById('launder-status').textContent = "No money to launder!";
            return;
        }
        
        // Laundering converts dirty money to clean money (no actual change in amount)
        addNotification(`Laundered $${amountToLaunder} through your businesses. Feds suspicion decreased.`);
        gameState.fedAttention = Math.max(0, gameState.fedAttention - 10);
        document.getElementById('launder-status').textContent = `Laundered $${amountToLaunder}`;
    });
}

// Street Selling Functions
function setupStreetSelling() {
    const offerSampleBtn = document.getElementById('offer-sample');
    const sellDrugBtn = document.getElementById('sell-drug');
    const upsellBtn = document.getElementById('upsell');
    const pickpocketBtn = document.getElementById('pickpocket');
    const nextCustomerBtn = document.getElementById('next-customer');
    
    offerSampleBtn.addEventListener('click', offerSample);
    sellDrugBtn.addEventListener('click', sellDrug);
    upsellBtn.addEventListener('click', upsellCustomer);
    pickpocketBtn.addEventListener('click', pickpocketCustomer);
    nextCustomerBtn.addEventListener('click', generateCustomer);
}

function generateCustomer() {
    if (gameState.drugInventory.length === 0) {
        document.getElementById('customer-info').innerHTML = '<p>No drugs to sell!</p>';
        document.getElementById('selling-status').textContent = 'You need drugs to attract customers';
        return;
    }
    
    const customerType = items.customerTypes[Math.floor(Math.random() * items.customerTypes.length)];
    gameState.currentCustomer = {
        ...customerType,
        mood: 'neutral',
        trust: customerType.trust,
        suspicion: customerType.suspicion
    };
    
    document.getElementById('customer-info').innerHTML = `
        <h3>${gameState.currentCustomer.name}</h3>
        <p>Mood: ${gameState.currentCustomer.mood}</p>
        <p>Trust: ${gameState.currentCustomer.trust}%</p>
    `;
    
    document.getElementById('selling-status').textContent = '';
}
...
function offerSample() {
    ...
    if (gameState.drugInventory.length === 0) {
        document.getElementById('selling-status').textContent = 'No drugs to give as sample!';
        return;
    }
    
    // Find cheapest drug
    const cheapestDrug = gameState.drugInventory.reduce((prev, current) => 
        (prev.basePrice < current.basePrice) ? prev : current
    );
    
    // Give sample (remove 1 unit)
    const drugIndex = gameState.drugInventory.findIndex(d => d.name === cheapestDrug.name);
    gameState.drugInventory[drugIndex].amount -= 1;
    if (gameState.drugInventory[drugIndex].amount <= 0) {
        gameState.drugInventory.splice(drugIndex, 1);
    }
    
    // Increase trust
    gameState.currentCustomer.trust = Math.min(100, gameState.currentCustomer.trust + 20);
    gameState.currentCustomer.mood = 'happy';
    ...
    document.getElementById('selling-status').textContent = `Gave free sample of ${cheapestDrug.name}. Customer trust increased!`;
    updateUI();
}

function sellDrug() {
    if (!gameState.currentCustomer || gameState.drugInventory.length === 0) return;
    
    // Select random drug
    const randomDrug = gameState.drugInventory[Math.floor(Math.random() * gameState.drugInventory.length)];
    const price = randomDrug.basePrice;
    
    // Check if customer will buy
    const buyChance = gameState.currentCustomer.trust - (price / 10);
    if (Math.random() * 100 > buyChance) {
        document.getElementById('selling-status').textContent = `${gameState.currentCustomer.name} refused to buy ${randomDrug.name}.`;
        gameState.currentCustomer.trust -= 10;
        gameState.currentCustomer.mood = 'angry';
        return;
    }
    ...
    // Remove drug
    const drugIndex = gameState.drugInventory.findIndex(d => d.name === randomDrug.name);
    gameState.drugInventory[drugIndex].amount -= 1;
    if (gameState.drugInventory[drugIndex].amount <= 0) {
        gameState.drugInventory.splice(drugIndex, 1);
    }
    ...
    if (gameState.gang && gameState.gang.name === 'Blood Vipers') {
        const bonus = Math.floor(saleAmount * 0.3);
        gameState.cash += bonus;
        addNotification(`Gang bonus: +$${bonus}`);
    }
    
    document.getElementById('selling-status').textContent = `Sold ${randomDrug.name} for $${saleAmount}!`;
    gameState.currentCustomer.trust += 5;
    gameState.currentCustomer.mood = 'happy';
    updateUI();
}

function upsellCustomer() {
    if (!gameState.currentCustomer || gameState.drugInventory.length === 0) return;
    
    // Select random drug
    const randomDrug = gameState.drugInventory[Math.floor(Math.random() * gameState.drugInventory.length)];
    const price = Math.floor(randomDrug.basePrice * 1.5); // 50% markup
    
    // Check if customer will buy
    const buyChance = gameState.currentCustomer.trust - (price / 5);
    if (Math.random() * 100 > buyChance) {
        document.getElementById('selling-status').textContent = `${gameState.currentCustomer.name} refused the upsell and got suspicious!`;
        gameState.currentCustomer.trust -= 15;
        gameState.currentCustomer.suspicion += 20;
        gameState.currentCustomer.mood = 'angry';
        
        // Chance to get caught
        if (Math.random() * 100 < gameState.currentCustomer.suspicion) {
    ...
    // Gang bonus
    if (gameState.gang && gameState.gang.name === 'Blood Vipers') {
        const bonus = Math.floor(price * 0.3);
        gameState.cash += bonus;
        addNotification(`Gang bonus: +$${bonus}`);
    }
    
    document.getElementById('selling-status').textContent = `Upsold ${randomDrug.name} for $${price}!`;
    gameState.currentCustomer.trust -= 5;
    gameState.currentCustomer.mood = 'neutral';
    updateUI();
}

function pickpocketCustomer() {
    if (!gameState.currentCustomer) return;
    
    const successChance = 70 - gameState.currentCustomer.suspicion;
    if (Math.random() * 100 > successChance) {
        // Failed pickpocket
        document.getElementById('selling-status').textContent = `${gameState.currentCustomer.name} caught you pickpocketing!`;
        gameState.currentCustomer.trust = 0;
        gameState.currentCustomer.suspicion = 100;
        gameState.currentCustomer.mood = 'angry';
        
        // High chance to get caught
        if (Math.random() * 100 < 80) {
            getArrested("Pickpocketing");
        }
        return;
    }
    
    // Successful pickpocket
    const stolenAmount = Math.floor(gameState.currentCustomer.cash * (0.1 + Math.random() * 0.3));
    gameState.cash += stolenAmount;
    
    document.getElementById('selling-status').textContent = `Pickpocketed $${stolenAmount} from ${gameState.currentCustomer.name}!`;
    gameState.currentCustomer.suspicion += 30;
    updateUI();
}
...
function setupJailAndCourt() {
    ...
    function updateCourt() {
        ...
        caseDetails.innerHTML = `
            ...
            <p><strong>Severity:</strong> ${'â­'.repeat(gameState.currentCase.severity)}</p>
