...
function setupPoker() {
    ...
    callBtn.addEventListener('click', () => {
        ...
        if (amountToCall > gameState.cash) {
            addNotification("Not enough cash to call!");
            return;
        }
        
        gameState.cash -= amountToCall;
        playerBet = currentBet;
        pot += amountToCall;
        
        addNotification(`You called $${amountToCall}.`);
        gameStatus = 'ai-turn';
        updateUI();
        updateDisplay();
        aiActions();
    ...
    raiseBtn.addEventListener('click', () => {
        ...
        const raiseAmount = parseInt(prompt("Enter raise amount:", currentBet + 100));
        if (isNaN(raiseAmount) || raiseAmount <= currentBet) {
            addNotification("Invalid raise amount!");
            return;
        }
        
        const totalBet = playerBet + (raiseAmount - currentBet);
        if (totalBet > gameState.cash) {
            addNotification("Not enough cash for that raise!");
            return;
        }
        
        gameState.cash -= (raiseAmount - playerBet);
        currentBet = raiseAmount;
        playerBet = raiseAmount;
        pot += (raiseAmount - playerBet);
        
        addNotification(`You raised to $${raiseAmount}.`);
        gameStatus = 'ai-turn';
        updateUI();
        updateDisplay();
        aiActions();
    ...
    foldBtn.addEventListener('click', () => {
        ...
        addNotification("You folded. Game over.");
        gameStatus = 'ended';
    });
    
    // Check
    checkBtn.addEventListener('click', () => {
        if (gameStatus !== 'player-turn' || currentBet > playerBet) {
            addNotification("Can't check when there's a bet!");
            return;
        }
        
        addNotification("You checked.");
        gameStatus = 'ai-turn';
        updateDisplay();
        aiActions();
    });
...
function setupSlotMachine() {
    ...
    const paytable = document.getElementById('slot-paytable');
    ...
    spinBtn.addEventListener('click', () => {
        ...
        reels.forEach((reel, index) => {
            const spins = 10 + index * 3; // Staggered stopping
            let spinCount = 0;
            
            const spinInterval = setInterval(() => {
                const randomSymbol = items.slotSymbols[
                    Math.floor(Math.random() * items.slotSymbols.length)
                ];
                ...
                if (spinCount >= spins) {
                    ...
                    // Last reel stopped - check for wins
    ...
    function checkSlotWin(bet) {
        ...
        if (winAmount > 0) {
            gameState.cash += winAmount;
            addNotification(`You won $${winAmount.toLocaleString()}!`);
            
            // Gang bonus
            if (gameState.gang && gameState.gang.name === 'Shadow Syndicate') {
                const bonus = Math.floor(winAmount * 0.2);
                gameState.cash += bonus;
                addNotification(`Gang bonus: +$${bonus.toLocaleString()}`);
            }
        } else {
